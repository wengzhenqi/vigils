...
- **浏览器内默认扫描**: 原文在页面侧由轻量 JS 规则识别；命中后由内容脚本直接给出阻断、脱敏或继续的页面内反馈
- **原文不落库**: 原文不写 `chrome.storage`、不写 `console.log`、不写 `window.*`；`chrome.storage` 只保存自定义网站 host/pattern 元数据
...
- **大小上限前置拦截**: 浏览器内规则先做字符长度早退兜底，避免把明显过大的文本继续送入后续判断（`too_large_sw` reason code）
- **三态执行**: service worker / content script 按 `allow`、`redact`、`block` 三态原样处理；非法 action / 协议错误帧 → fail-closed `block`
