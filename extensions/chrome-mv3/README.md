...
Chrome MV3 扩展,I09b-α1 提供 paste / submit 守门 + 与本机 Native Host(`com.vigil.host`)通信。

## 当前范围(I09b-α1)

- ✅ `manifest.json`:真实 MV3 配置(name / version 0.1.0 / host_permissions / content_scripts / CSP);permissions **最小化仅 `nativeMessaging`**(R1 MUST-FIX 3 削减)
...
- ✅ `content-script.js`:4 个 host(ChatGPT / Claude / Gemini / Perplexity)paste + submit + contenteditable Enter 三路径 + 简易 `textContent` toast;submit allow 走 `form.requestSubmit(submitter)` + `WeakSet` allow-once(R1 MUST-FIX 1 保留 HTML validation + 其他 submit listener 参与)
...
3. 访问 4 host 之一,touch paste / submit → 若 Native Host 未注册,应看到 "Vigil: 粘贴被阻断(host_disconnected)"(fail-closed)
