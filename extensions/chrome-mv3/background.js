...
// **不加 storage permission**(继续 α4 最小权限原则);Stage 2+ 可用 Native Host config
// 持久化(RPC 启动时拉一次);当前 MVP 接受 session 级设置 + 持续设置留给后续 ISS。
//
...
// 不落 chrome.storage(需要 storage permission,违最小权限 β1 教训),不记原文(ADR §I-9.1),
