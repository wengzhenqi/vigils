...
**最快(macOS / Linux)** —— 一行装好 CLI,然后直接看[快速开始](#快速开始):
...
把 `vigil-hub` CLI 装到 `~/.local/bin`。它**只把二进制放到磁盘** —— 不改 shell 配置、不自动 `setup`、
不碰任何 agent 配置 —— 并打印接下来该做什么,你始终掌控。想先读再 pipe 给 shell?脚本在
[`install.sh`](./install.sh)。Windows 或想手动下载?见[安装](#安装)。
